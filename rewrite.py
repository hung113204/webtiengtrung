...
file_path = r'c:\xampp\htdocs\webtiengtrung\resources\views\frontend\khoahocclient\index.blade.php'
...
# Extract JS
js_start = content.find('document.addEventListener("DOMContentLoaded", function () {')
js_end = content.find('</script>', js_start)
if js_start != -1 and js_end != -1:
    js_content = content[js_start:js_end]
else:
    print("Could not find js content")
    exit(1)

# Clean JS
js_content = re.sub(r'/\* ---------- Theme ---------- \*/.*?/\* ---------- Save / bookmark toggle ---------- \*/', '/* ---------- Save / bookmark toggle ---------- */', js_content, flags=re.DOTALL)


new_content = f"""@extends('frontend.layouts.main')
